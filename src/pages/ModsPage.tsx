...
const ModsPage: React.FC = () => {
  ...
  const displayWishes = searchTerm.trim() ? filteredWishes : modWishes;
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div ref={searchRef} style={{ position: 'relative', marginBottom: 32 }}>
              <div style={{
                ...
                border: '1.5px solid rgba(50,205,50,0.35)',
                borderRadius: 14,
                boxShadow: '0 4px 24px rgba(0,0,0,0.35), 0 0 0 1px rgba(50,205,50,0.1)',
                overflow: 'hidden',
                transition: 'border-color 0.2s, box-shadow 0.2s',
              }}>
                <div style={{ paddingLeft: 16, display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                  <Search size={18} style={{ color: '#32CD32' }} />
                </div>
                <input
                  type="text"
                  value={searchTerm}
                  ...
                  placeholder="Buscar moderador por nombre..."
                  style={{
                ...
                <div style={{
                  padding: '8px 16px',
                  borderLeft: '1px solid rgba(50,205,50,0.2)',
                  background: 'rgba(50,205,50,0.1)',
                  color: '#32CD32', fontSize: 12, fontWeight: 700,
                  whiteSpace: 'nowrap',
                }}>
                  {modWishes.length} mods
                </div>
              </div>
              ...
              {dropdownOpen && searchTerm && (
                <div style={{
                  ...
                  border: '1.5px solid rgba(50,205,50,0.3)',
                  borderRadius: 12,
                  ...
                  {filteredWishes.length > 0 ? filteredWishes.map(wish => (
                    ...
                    onMouseEnter={e => (e.currentTarget.style.background = 'rgba(50,205,50,0.1)')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                    >
                      <img src={wish.image} alt={wish.name} style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '1.5px solid #32CD32', flexShrink: 0 }} />
                    ...
                    <div style={{ padding: '16px', color: '#64748b', fontSize: 14, textAlign: 'center' }}>
                      No se encontró ningún moderador con ese nombre.
                    </div>
                  )}
                </div>
              )}
            ...
            {displayWishes.length > 0 ? (
              <div className="space-y-6">
                {displayWishes.map(wish => (
                  <div key={wish.id} id={wish.id}
                    style={selectedWish === wish.id ? { outline: '2px solid #32CD32', borderRadius: 16, boxShadow: '0 0 0 4px rgba(50,205,50,0.2)' } : {}}>
                    <WishCard wish={wish} />
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-12 bg-white/90 backdrop-blur-sm rounded-xl">
                <p className="text-gray-500 text-lg">
                  {searchTerm ? 'No se encontraron moderadores con ese nombre.' : 'Aún no hay felicitaciones de moderadores.'}
                </p>
